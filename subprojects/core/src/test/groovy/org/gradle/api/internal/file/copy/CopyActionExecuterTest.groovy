...
class CopyActionExecuterTest extends WorkspaceTest {
    ...
    def "correctly executes copy actions, normalising and handling excludes"() {
        ...
        def copySpec = TestUtil.newInstance(DestinationRootCopySpec, resolver, new DefaultCopySpec(fileCollectionFactory, TestUtil.propertyFactory(), TestUtil.instantiatorFactory().decorateLenient(), TestFiles.patternSetFactory))
        copySpec.with {
            into "out"
            from "a", {
                from "b/b1", {
